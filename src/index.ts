...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Brave Search');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    case 'web_search':
      return webSearch(apiKey, { ...args, q: reqStr(args, 'q', '"openai ipo timeline"') });
    case 'news_search':
      return newsSearch(apiKey, { ...args, q: reqStr(args, 'q', '"FOMC rate decision"') });
    default:
